...
def build_analysis(engine: OSISEngine) -> None:
    ...
    engine.live.lane.create("车道1", "VE", 47.0, 1.8, 1, 0, "车道1车道线单元组", 4.725, 0.0)
    engine.live.lane.create("车道2", "VE", 47.0, 1.8, 1, 0, "车道2车道线单元组", 1.625, 0.0)
    engine.live.lane.create("车道3", "VE", 47.0, 1.8, 1, 0, "车道3车道线单元组", -1.475, 0.0)
    engine.live.lane.create("车道4", "VE", 47.0, 1.8, 1, 0, "车道4车道线单元组", -4.575, 0.0)
    # 创建活载工况
    ...
    engine.live.case.get("移动荷载工况").include("a", "移动荷载工况_sub1", "CH-CD", 1.0, 1, "CUSTOM", 0.85, "车道1", "车道2", "车道3", "车道4")
    # 设置活载子工况的加载车道数范围
    engine.live.case.get("移动荷载工况").set_lane_count("移动荷载工况_sub1", 0, 4)
    # 创建或修改荷载转换质量总体信息。
