...
def build_boundaries(engine: OSISEngine) -> None:
    ...
    # 分配边界给节点
    engine.boundary.get(1).assign("a", 26)
    engine.boundary.create(2, "GENERAL", "", 0, 1, 1, 0, 0, 0, 0)
    engine.boundary.get(2).assign("a", 2)
    engine.boundary.create(3, "GENERAL", "", 0, 1, 1, 0, 0, 0, 0)
    engine.boundary.get(3).assign("a", 51)
    engine.boundary.create(4, "GENERAL", "", 0, 1, 1, 0, 0, 0, 0)
    engine.boundary.get(4).assign("a", 75)
    # 创建边界组
    engine.boundary.group.create("成桥", "c")
    engine.boundary.group.create("成桥", "a", "1to4")
    engine.boundary.group.create("弹性连接", "c")
    engine.boundary.group.create("刚性连接", "c")
