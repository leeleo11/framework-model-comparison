...
def build_boundaries(engine: OSISEngine) -> None:
    ...
    # 分配边界给节点
    engine.boundary.get(1).assign("a", 2, 28, 54)
    engine.boundary.create(2, "GENERAL", "", 0, 1, 1, 1, 0, 1, 0)
    engine.boundary.get(2).assign("a", 24, 50, 76)
    ...
    engine.boundary.get(4).assign("a", 26)
    engine.boundary.create(5, "GENERAL", "", 0, 1, 1, 1, 0, 1, 0)
    engine.boundary.get(5).assign("a", 52)
    engine.boundary.create(6, "GENERAL", "", 0, 1, 1, 1, 0, 1, 0)
    engine.boundary.get(6).assign("a", 76)
    # 创建边界组并返回刷新后的对象
