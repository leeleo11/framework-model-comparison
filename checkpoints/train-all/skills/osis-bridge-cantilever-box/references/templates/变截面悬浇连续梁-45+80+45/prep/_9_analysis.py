...
def build_analysis(engine: OSISEngine) -> None:
    # 创建或修改沉降组
    engine.settlement.group.create("桥墩1沉降组", -0.015, 66, 67)
    engine.settlement.group.create("桥墩2沉降组", -0.015, 68, 69)
    engine.settlement.group.create("桥台1沉降组", -0.012, 64, 65)
    engine.settlement.group.create("桥台2沉降组", -0.012, 70, 71)
    # 创建沉降荷载工况
    ...
    engine.live.lane.create("右偏载工况车道1", "VE", 80.0, 1.8, 1, 0, "主梁单元", -4.475, 0.0)
    engine.live.lane.create("右偏载工况车道2", "VE", 80.0, 1.8, 1, 0, "主梁单元", -1.375, 0.0)
    engine.live.lane.create("中载工况车道1", "VE", 80.0, 1.8, 1, 0, "主梁单元", -1.55, 0.0)
    engine.live.lane.create("中载工况车道2", "VE", 80.0, 1.8, 1, 0, "主梁单元", 1.55, 0.0)
    engine.live.lane.create("左偏载工况车道1", "VE", 80.0, 1.8, 1, 0, "主梁单元", 4.475, 0.0)
    engine.live.lane.create("左偏载工况车道2", "VE", 80.0, 1.8, 1, 0, "主梁单元", 1.375, 0.0)
    # 创建活载工况
    ...
    engine.live.case.get("右偏载工况").include("a", "右偏车道荷载子工况", "变截面悬浇梁移动荷载", 1.15, 1, "CUSTOM", 1.43, "右偏载工况车道1", "右偏载工况车道2")
    # 设置活载子工况的加载车道数范围
    ...
    engine.live.case.get("中载工况").include("a", "中载车道荷载子工况", "变截面悬浇梁移动荷载", 1.15, 1, "CUSTOM", 1.43, "中载工况车道1", "中载工况车道2")
    ...
    engine.live.case.get("左偏载工况").include("a", "左偏车道荷载子工况", "变截面悬浇梁移动荷载", 1.15, 1, "CUSTOM", 1.43, "左偏载工况车道1", "左偏载工况车道2")
