...
def build_analysis(engine: OSISEngine) -> None:
    ...
    engine.settlement.group.create("沉降组2", -0.005, 22)
    engine.settlement.group.create("沉降组3", -0.005, 44)
    engine.settlement.group.create("沉降组4", -0.005, 64)
    # 创建沉降荷载工况
    ...
    engine.live.case.get("车道荷载包络").include("a", "车道荷载工况1", "简支变连续T梁移动荷载", 0.77, 1, "CUSTOM", 3.522, "车道")
    # 设置活载子工况的加载车道数范围
