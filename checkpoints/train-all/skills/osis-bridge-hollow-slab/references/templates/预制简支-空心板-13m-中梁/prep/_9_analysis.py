...
def build_analysis(engine: OSISEngine) -> None:
    # 创建或修改沉降组
    engine.settlement.group.create("沉降组1", -0.05, 2)
    engine.settlement.group.create("沉降组2", -0.05, 12)
    # 创建沉降荷载工况
    engine.settlement.create("沉降工况")
    # 将沉降组添加至当前工况
    engine.settlement.get("沉降工况").include("a", "沉降组1", "沉降组2")
    # 创建活载等级（便捷入口，内部转发到对应 create_* 方法）
    engine.live.grade.create("简支空心板移动荷载", "JTGD60_2015", "HIGHWAY_I")
    # 创建车道（便捷入口，内部转发到对应 create_* 方法）
    engine.live.lane.create("车道", "VE", 12.08, 1.8, 1, 0, "主梁单元", 0.0, 0.0)
    # 创建活载工况
    ...
    engine.live.case.get("车道荷载包络").include("a", "车道荷载工况1", "简支空心板移动荷载", 0.25, 1, "CUSTOM", 10.4191, "车道")
    # 设置活载子工况的加载车道数范围
