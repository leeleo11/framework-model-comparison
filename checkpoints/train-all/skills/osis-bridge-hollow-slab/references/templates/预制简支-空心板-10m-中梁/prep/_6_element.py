...
def build_elements(engine: OSISEngine) -> None:
    ...
    # 分配或重置单个单元的理论厚度
    engine.prop.assign_component_thickness(0.2269, "a", 3, 12)
    engine.prop.assign_component_thickness(0.289, "a", "1to2", "13to14")
    engine.prop.assign_component_thickness(0.1886, "a", "4to11")
    # 创建单元组
    engine.element.group.create("封端混凝土单元", "c")
    engine.element.group.create("封端混凝土单元", "a", 1, 14)
