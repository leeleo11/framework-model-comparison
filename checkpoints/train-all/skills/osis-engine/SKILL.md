...
| 模板匹配 | 本文件 §模板优先策略(`seedtpl` / 全量名单),各 bridge `references/templates/` |
...
- **默认求快**:用户没另说时走最快路径——能复制模板不构思、能最小 diff 不重排、能 L0 不 L1、能 L1 不 L2;未指定参数用模板/桥型默认,完工报告列假设请用户纠正,不逐项反问。求快不裁三样:**算术闭合自检**、**构造评测**(本条末)、**桥型歧义时的澄清**
...
按**受力体系 + 施工方法**识别,加载唯一对应的桥型 SKILL:
...
路由桥型后先跑 `seedtpl`。stdout 的 `共 N 个模板: [...]` 即全量名单;**未报全量不得宣布命中/近邻**。N 对不上或名单像截断 → 再跑,或 `ls` 当前桥型 `references/templates/`。只查本桥型平铺目录(没有 `<桥型>/<跨径>` 两级);刚构不在悬浇库。

```bash
python "%OSIS_EXTRA_CONFIG_DIR%\skills\osis-engine\scripts\seedtpl.py" --spec "<用户原话>"
```

复制只用 `seedtpl`(目标 `get_directory()/py/`),不要 `cp`、不要打开模板逐行重写。
- 用户消息已有 `[OSIS-SEEDTPL] 已把最近模板复制` → 不要再跑 `seedtpl`,在该 `py/prep` 上改
- 用户消息已有 `[OSIS-SEEDTPL]` 且写明工程已有模型/未覆盖 → **不要再跑 `seedtpl`**。先读画像说明现有桥型,问是否覆盖;用户明确确认后再 `seedtpl --force`
